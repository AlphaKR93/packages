...
def main():
    ...
    for glob in exclude_globs:
        excludes |= set(str(excluded.relative_to(src_module)) for excluded in src_module.rglob(glob) if str(excluded.relative_to(src_module)).startswith(glob.split(os.sep)[0]))

    src_excludes = set()
    for exclude_glob in src_exclude_globs:
        src_excludes |= set(str(excluded.relative_to(src)) for excluded in src.rglob(exclude_glob) if str(excluded.relative_to(src)).startswith(exclude_glob.split(os.sep)[0]))
